...
def _train(model, args):
    """训练主函数(去雾版):使用空间域 L1 损失 + 频率域 FFT 损失的多尺度监督,
    配合 CosineAnnealing + 预热(warmup)学习率调度。"""
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    criterion = torch.nn.L1Loss()   # 空间/频率域都用 L1 损失

    optimizer = torch.optim.Adam(model.parameters(), lr=args.learning_rate, betas=(0.9, 0.999), eps=1e-8)
    dataloader = train_dataloader(args.data_dir, args.batch_size, args.num_worker, args.data)  # 训练时传入数据集类型(args.data)
    ...
    for epoch_idx in range(epoch, args.num_epoch + 1):
        ...
        for iter_idx, batch_data in enumerate(dataloader):
            ...
            loss.backward()
            torch.nn.utils.clip_grad_norm_(model.parameters(), 0.001)  # 梯度裁剪防爆炸(去雾版阈值 0.001,小于主副本的 0.01)
            optimizer.step()
