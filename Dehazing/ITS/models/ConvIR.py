...
class EBlock(nn.Module):
    ...
    与主副本差异:多一个 data 参数,向下传导给每个 ResBlock。"""
...
class ConvIR(nn.Module):
    """ConvIR 主网络(去雾版):编码器-解码器 U 型结构 + 多尺度监督输出。
    与主副本差异:构造时需传入 version(决定残差块个数)与 data(决定空洞率)。"""
    def __init__(self, version, data):
        super(ConvIR, self).__init__()
        
        # 根据 version 设定残差块个数:small=4, base=8, large=16
        if version == 'small':
            num_res = 4
        elif version == 'base':
            num_res = 8
