...
class ResBlock(nn.Module):
    ...
    与主副本差异:多了一个 data 参数,用于决定 DeepPool 的空洞率(针对不同去雾数据集),并向下传导。"""
    def __init__(self, in_channel, out_channel, data, filter=False):
        super(ResBlock, self).__init__()
        self.main = nn.Sequential(
            BasicConv(in_channel, out_channel, kernel_size=3, stride=1, relu=True),
            DeepPoolLayer(in_channel, out_channel, data) if filter else nn.Identity(),  # filter 开关控制是否插入深层池化滤波
...
class DeepPoolLayer(nn.Module):
    ...
    与主副本差异:根据 data 数据集类型选择空洞率(dilation)列表。"""
    def __init__(self, k, k_out, data):
        super(DeepPoolLayer, self).__init__()
        self.pools_sizes = [8,4,2]   # 三种下采样的池化窗口大小

        # 根据数据集选择空洞率:ITS 及相关去雾数据用 [7,9,11],GTA5 用 [5,9,11]
