...
def _eval(model, args):
    """测试函数(OTS 版):加载指定权重,计算 PSNR 与 SSIM(pytorch_msssim),
    可选保存输出图像,并统计平均推理时间。
    与 ITS 版差异:用 cuda.synchronize 保证计时更准确。"""
    ...
    with torch.no_grad():      # 测试阶段关闭梯度
        ...
        for iter_idx, data in enumerate(dataloader):
            ...
            torch.cuda.synchronize()   # 等待 GPU 队列执行完毕,确保计时起点准确
            tm = time.time()   # 计时开始

            pred = model(input_img)[2]   # 取原图尺度输出
            pred = pred[:,:,:h,:w]       # 裁回原始尺寸(去掉填充)
            torch.cuda.synchronize()     # 确保前向完成,计时终点准确
        ...
        print('The average SSIM is %.4f dB' % (ssim_adder.average()))   # 平均 SSIM
