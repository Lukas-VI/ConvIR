...
def _train(model, args):
    """训练主函数(去雾 OTS 版):空间 L1 + 频率 FFT 的多尺度监督 + cosine warmup。
    与 ITS 版差异:warmup=1 epoch、训练中按迭代周期做一次中间验证(eval_now 块)。"""
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    criterion = torch.nn.L1Loss()   # 空间/频率域都用 L1 损失

    optimizer = torch.optim.Adam(model.parameters(), lr=args.learning_rate, betas=(0.9, 0.999), eps=1e-8)
    dataloader = train_dataloader(args.data_dir, args.batch_size, args.num_worker)  # OTS 版不传 data 参数
    ...
    eval_now = max_iter//6-1            # 训练中每隔这些迭代做一次中间验证(OTS 版特色)
    ...
    for epoch_idx in range(epoch, args.num_epoch + 1):
        ...
        for iter_idx, batch_data in enumerate(dataloader):
            ...
            # 训练过程中间验证(OTS 版):每 eval_now 迭代且过了前 20 个 epoch(或第1个epoch)就验证一次并保存
            if iter_idx%eval_now==0 and iter_idx>0 and (epoch_idx>20 or epoch_idx == 1):
                ...
                torch.save({'model': model.state_dict()}, save_name)   # 保存当前检查点

                val_gopro = _valid(model, args, epoch_idx)             # 用 GOPRO/OTS 测试集验证
