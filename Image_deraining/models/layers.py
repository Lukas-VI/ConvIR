...
class ResBlock(nn.Module):
    ...
    若 filter=True,则在中间插入 DeepPoolLayer 做多尺度动态滤波增强。"""
...
class MultiShapeKernel(nn.Module):
    """多形状内核(条状 + 方形)的动态滤波,是 ConvIR 的核心特征提取单元。
    结合了立方条状注意力(strip)与方形动态滤波(square),通过相加融合两种形状感受野。"""
    def __init__(self, dim, kernel_size=3, dilation=1, group=8):
        ...
        self.strip_att = cubic_attention(dim, group=group, dilation=dilation, kernel=kernel_size)                 # 条状动态注意力

    def forward(self, x):
        x1 = self.strip_att(x)    # 条状分支
        x2 = self.square_att(x)   # 方形分支
        return x1+x2              # 两种感受野结果相加